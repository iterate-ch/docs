...
The cache directory is located in `%LocalAppData%\Cyberduck\Cache` on Windows or in *Application Support folder* on macOS by default. You can [choose another disk](Preferences#sync-cache-location) for storing files in the local cache. You can clear cached files from the local disk with the *Delete on local disk* [context menu](Sync#delete-on-local-disk) option.
