...
Changes from the server are not immediately visible. The directory listing in the file browser may become outdated when another application adds, removes, or modifies files on the server. Choose *Reload* from the context menu in _Finder_ or _Windows Explorer_ to refresh the directory listing. On Windows, you can also use the _F5_ key in _Windows Explorer_ to refresh the file listing.
