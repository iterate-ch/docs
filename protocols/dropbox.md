...
Alternatively, you can reset the OAuth token by deleting the entries related to `duck:dropbox?user=(user)` out of the *Windows Credential Manager* or on macOS the entries related to `https://Dropbox (user)@api.dropboxapi.com` out of *Keychain Access.app*.
